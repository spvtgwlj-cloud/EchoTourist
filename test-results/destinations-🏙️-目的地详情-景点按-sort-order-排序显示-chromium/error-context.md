...
- Name: destinations.spec.ts >> 🏙️ 目的地详情 >> 景点按 sort_order 排序显示
- Location: tests/e2e/destinations.spec.ts:102:7

# Error details

```
Error: page.goto: net::ERR_CONNECTION_REFUSED at http://localhost:3000/zh/destinations/beijing
Call log:
  - navigating to "http://localhost:3000/zh/destinations/beijing", waiting until "load"
...
> 103 |     await page.goto('/zh/destinations/beijing');
  ...
  116 |   test('从导航栏进入目的地页面', async ({ page }) => {
  117 |     await page.goto('/zh');
  118 |     await page.locator('header a[href*="destinations"]').first().click();
  119 |     await page.waitForURL('**/destinations');
  120 |     await expect(page.locator('h1, h2').first()).toBeVisible();
  121 |   });
  122 | 
  123 |   test('首页目的地预览可点击', async ({ page }) => {
  124 |     await page.goto('/zh');
  125 |     await page.waitForLoadState('networkidle');
  126 | 
  127 |     const destPreviewLink = page.locator('a[href*="/zh/destinations/"]').first();
  128 |     if (await destPreviewLink.isVisible({ timeout: 5000 }).catch(() => false)) {
  129 |       await destPreviewLink.click();
  130 |       await expect(page).toHaveURL(/\/zh\/destinations\//);
  131 |     }
  132 |   });
  133 | });
  134 | 
