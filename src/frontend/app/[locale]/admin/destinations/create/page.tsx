...
export default function AdminCreateDestinationPage() {
  ...
  const handleSubmit = async () => {
    ...
    const hasName = translations.some(t => t.name.trim());
    if (!hasName) { setError('At least one translation must have a name'); return; }

    setSaving(true);
    setError('');
    try {
      const res = await api.post<{ status: string; id: string }>('/admin/destinations', {
        slug: slug.trim().toLowerCase().replace(/\s+/g, '-'),
        image_url: imageUrl || null,
        status,
        translations: translations.filter(t => t.name.trim()).map(t => ({
          locale: t.locale,
          name: t.name.trim(),
      ...
      setError(e?.message || 'Failed to create destination');
    } finally {
      setSaving(false);
    }
  };
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">Unique identifier used in URLs (auto-lowercased)</p>
            </div>
            <div className="space-y-2">
              <Label>Image URL</Label>
              <Input
                placeholder="/images/destinations/city.svg"
                value={imageUrl}
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      placeholder={`Description in ${trans.locale}`}
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button onClick={handleSubmit} disabled={saving}>
            {saving ? (
              ...
              <><Save className="h-4 w-4 mr-2" /> Create Destination</>
            )}
          </Button>
        </div>
      </div>
